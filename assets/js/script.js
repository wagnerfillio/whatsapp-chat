...
let viewChatList = () => {
	...
	.forEach((elem, index) => {
		...
		DOM.chatList.innerHTML += `
		<div id="chat-item" class="chat-list-item d-flex flex-row w-100 p-2 border-bottom-2 _border-bottom ${unreadClass}" onclick="generateMessageArea(this, ${index})">
			<img src="${elem.isGroup ? elem.group.pic : elem.contact.pic}" alt="Profile Photo" class="img-fluid rounded-circle mx-2 mr-2" style="height:50px;">
			<div class="w-50">
...
let addMessageToMessageArea = (msg) => {
	...
	DOM.messages.innerHTML += `
	<div class="align-self-${msg.sender === user.id ? "end self" : "start"} p-1-apagar my-1 mx-5 rounded-apagar bg-white shadow-sm-apagar rounded-7-5 m-p-r m-shadow message-item">
...
let generateMessageArea = (elem, chatIndex) => {
	...
	if (chat.isGroup) {
		...
		let memberNames = contactList
				...
				.map(contact => contact.id === user.id ? "Você" : contact.name)
				.join(", ");
